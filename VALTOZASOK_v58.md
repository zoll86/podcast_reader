...
## 1. Tanulói kérések: TELJESEN kikapcsolva
A cél most a felismerés és a fordítás tökéletesítése — tanulói (szópár-)
kérés SEMMILYEN úton nem megy ki: a rendbetételből is kikerült a szópár-lépés,
a tanuló mód bekapcsolása sem gyűjt semmit (a meglévő párokkal megy, és szól,
hogy a gyűjtés szünetel). A gépezet (fillPairsRec, llmPairs) megvan, de sehova
nincs bekötve — később, a tanuló módos körben tér vissza.

## 2. Borítónézet a hosszú listákban
- A lista ⋯ lapján a részek sorai elé odakerült a BORÍTÓ-miniatűr (44px) —
  a hosszú lista képpel pásztázható.
- Új kapcsoló ugyanott: „borítónézet a polcon (részek csempéi)" — a lista
  kibontva ugyanazt a csempés nézetet adja, mint kinn a polc. (Ez eddig is
  létezett a kártya „N rész" pöttyével, csak rejtve — most ki van mondva.)

---

# VÁLTOZÁSOK — v56 → v57: A FELDOLGOZÁS GYORS ÚJRA

A napló két tettest mutatott, mindkettő javítva.

## 1. Az automatikus szópár-menet KIKERÜLT a fordítási útból
A tanuló mód bekapcsolása egyszer örökre beállította a szópár-gyűjtést, és a
v52 óta ettől MINDEN fordítási korty után ment egy második kérés is — a
feldolgozás megduplázódott, és a második kérés sokszor csonka JSON-nal kárba
veszett („szópárok pótlása: Unexpected end of JSON input"). Mostantól szópár
KIZÁRÓLAG kérésre készül: ⋯ → „szöveg rendbetétele" (jóváhagyással,
folyamatjelzővel, 5-ös kötegekben, hogy ne csonkuljon). A fordítás útja
tiszta: egy kérés = fordítás.

## 2. A fordító válaszkerete a bemenethez skálázódik
A v53 óta az egység TELJES mondat, és 10 hosszú mondat magyarja nem fért a
fix 1400 tokenes keretbe — a JSON csonkult („fordító (claude): Unexpected end
of JSON input"), az egész köteg kárba veszett és újrapróbálódott. A keret most
a bemenet hosszához igazodik (800–4000 token), kétszeres ráhagyással.

---

# VÁLTOZÁSOK — v55 → v56: KÉNYELEM ÉS DESIGN

## 1. A borító filter nélkül él
A lejátszó hátterében a borítón eddig egy erős sötétítő átmenet ült (50→96%),
tanuló módban plusz egy réteg. Mindkettő lekerült: az olvashatóságot a
buborékok saját fekete alapja adja (chipek a sorok mögött, kártya az
aktuálison) — a kép filter nélkül, a maga színeivel látszik.

## 2. A Behúzás-kártya csukható
Ritka művelet, mégis a könyvtár alsó felét foglalta. Telefonon csukva indul
(egy sor, a címre koppintva nyílik/csukódik), asztali gépen — ahol tényleg van
egérrel húzás — nyitva marad.

## 3. Beállítások: a mindennapi elöl, az egyszeri csukva
A Megjelenés (betű, ráhagyás, magyar sáv, immerzív) került legfelülre. A
Beszédfelismerés és a Fordítás motor/kulcs-kártyák a „Motorok és kulcsok —
egyszeri beállítás" csoport mögé kerültek, ami koppintásra nyílik; kulcs
nélkül (első használat) nyitva indul, hogy meglegyen az út.

## 4. Könyvtári kártya apróságok
- A ⋯ pötty találati felülete 26 → 40 px (a rajza alig nőtt, a célja igen).
- A „0%" jelvény nem hír, lekerült: jelvény csak folyamatban lévő
  feldolgozásnál (1–98%); szöveg nélküli felvételnél a hiány kimondva:
  „szöveg ✗" (a „hang ✗" mintájára).
